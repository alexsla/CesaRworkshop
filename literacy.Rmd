...
*\textcolor{olive}{Hint: are you looking in the right working directory? The function `getwd()` can help you figure out where R is searching for the file. You may want to change the working directory using `setwd()`, or provide a full pathway to the file you're trying to import}*

*\textcolor{olive}{Hint: use <- to store an object}*
...
You've just created a matrix! We've put in six numbers, and asked to generate a matrix with 2 columns. Therefore, there are 2 columns. What happens if you try to run the following code?

```{r matrix2, eval = FALSE}
matrix(c(1,2,3,4,5,6,7), ncol = 2)
```

All columns of matrix (and a data.frame) must have the same length, as do all rows. In this chunk of code we tried to create a matrix with 2 rows from 7 numbers - and 7 is not divisible by two. So what happened here?
...
The output you have just printed is a vector, which is also the second column of the matrix. The function `mat[i,]` extracts the *i*th row of matrix *mat*, and the function `mat[,k]` extracts the *k*th column of matrix *mat*.
...
What are data types? There are several data types in R, the most basic ones, and which we will use the most, are numeric (1, 2.5, 77, etc.), character (blah, ok, "wow how cool"), and logical (TRUE, FALSE). Quotation marks are used to show that something is a character - this can work for numerics that are coerced to be a character ("1" vs 1), or for longer strings that include spaces (like "wow how cool", which can be a single element of vector, as opposed to wow how cool which will create an error - R really doesn't like spaces). Numerics can be coerced into characters, characters can't be coerced into numerics. So, since we supplied a mixed vector to `mat`, every element there was coerced into a character. We can see this by running the following code:
...
We can also coerce vectors manually. For example, we can coerce the first column of `df` into a character, like in `mat` by running `as.character(df$col1)`. We can also coerce the first column of `mat` into a numeric by running `as.numeric(mat[,1])`.
...
By default, `mean()` returns `NA` if the input includes any missing value. Luckily, we can set the `na.rm` argument to `TRUE` to tell the function to remove all missing values from the input before caulcating the mean. Therefore, if you run the following code, you should get the correct mean for the column:
...
*\textcolor{olive}{Hint: use `str()` to see the column names}*

### Explanation:
Remember, R **really** doesn't like spaces. Our original csv file had the column named column c - which R automatically changed into column.c. Follow iSOP for naming conventions for columns to avoid issues like this.
...
If you've reached this far, you have succesfully completed the Cesar R literacy quiz! This means you are now ready to participate in the Cesar R workshop. Until then, time to celebRate!